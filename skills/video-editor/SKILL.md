...
- Generate editing MP4 versions through `MediaAssetCatalog.generatePreviewVersion()`. Pass an `AbortSignal`, surface non-cancellation errors, and call `renderer.refreshAssets()` after completion. Preview and compose resolvers prefer the current generated version and fall back to the original.
